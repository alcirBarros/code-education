package br.com.educafaciltemplate.config.openentityview;
...
/**
 * Servlet Filter that binds a JPA EntityManager to the thread for the entire
 * processing of the request. Intended for the "Open EntityManager in View"
 ...
 * Supports an "entityManagerFactoryBeanName" filter init-param in
 * {@code web.xml}; the default bean name is "entityManagerFactory". As an
 * alternative, the "persistenceUnitName" init-param allows for retrieval by
 * logical unit name (as specified in {@code persistence.xml}).
 *
...
public class OpenEntityManagerInViewFilter extends OncePerRequestFilter {
     ...
     * application context.
     * <p>
     * Default is "entityManagerFactory". Note that this default only applies
    ...
    /**
     * Set the name of the persistence unit to access the EntityManagerFactory
     * for.
     * <p>
     * This is an alternative to specifying the EntityManagerFactory by bean
     * name, resolving it by its persistence unit name instead. If no bean name
     * and no persistence unit name have been specified, we'll check whether a
     * bean exists for the default bean name "entityManagerFactory"; if not, a
     * default EntityManagerFactory will be retrieved through finding a single
    ...
    /**
     * Return the name of the persistence unit to access the
     * EntityManagerFactory for, if any.
     */
    @Nullable
     ...
     * {@code EntityManager} to each asynchronously dispatched thread and
     * postpone closing it until the very last asynchronous dispatch.
     */
    @Override
