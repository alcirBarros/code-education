...
import br.com.template.util.Constants;
...
public class AdminFilter implements Filter {
    ...
    @PostConstruct
    public void init() {
        System.out.println("br.com.template.session.AdminFilter.init()");
    }
    ...
    public void init(FilterConfig filterConfig) {
        ...
        adminConfig = initBean(AdminConfig.class, servletContext);
        adminSession = initBean(AdminSession.class, servletContext);
...
//            initBeans();
    ...
    private <T extends Object> T initBean(Class<T> classBean, ServletContext servletContext) {
        try {
            WebApplicationContext webApplicationContext = WebApplicationContextUtils.getWebApplicationContext(servletContext);
            return webApplicationContext.getBean(classBean);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    /**
     * Workaround for open web beans on tomcat, see
     * https://stackoverflow.com/questions/45205472/apache-openwebbeanscdi-servlet-injection-not-working
     */
...
//
        if (request.getRequestURI().equals(request.getContextPath() + "/") || (adminSession.isLoggedIn() && request.getRequestURI().endsWith(loginPage))) {
        ...
        if (skipResource(request, response) || adminSession.isLoggedIn()) {
            ...
            //resource not skipped (e.g a page that is not logon page) AND user not logged in
//            redirectToLogon(request, (HttpServletResponse) servletResponse);
            filterChain.doFilter(servletRequest, servletResponse);
        }
    }
    ...
    private void redirectToLogon(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (!requestedPage.contains(loginPage) && requestedPage.contains(".")) {
             ...
             */
            String redirectUrl = request.getContextPath() + "/" + loginPage + (has(recoveryUrl)  && isValidRecoveryUrl(recoveryUrl) ? "?page=" + URLEncoder.encode(recoveryUrl.toString(), "UTF-8") : "");
